...
export async function GET(req: Request) {
  try {
    ...
    const slidesWithUrls = slides.map((s: any) => {
      const slideData = s.toObject ? s.toObject() : s._doc || s;
      const imageUrl = slideData.image && !slideData.image.startsWith("http")
        ? getUrlFromPublicId(slideData.image)
        : slideData.image;
      
      console.log("Hero slide image conversion:", slideData.image, "->", imageUrl);
...
export async function POST(req: Request) {
  try {
    ...
    if (file && file instanceof File) {
      ...
      const result = await uploadToCloudinary(base64Image, "miraly/hero");
    ...
    const slideData = slide.toObject ? slide.toObject() : slide._doc || slide;
    const imageUrl = slideData.image && !slideData.image.startsWith("http")
      ? getUrlFromPublicId(slideData.image)
      : slideData.image;
    
    console.log("Created slide image conversion:", slideData.image, "->", imageUrl);
