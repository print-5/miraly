    ...
    <div className="space-y-8 pb-20">
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-6 bg-white p-5 sm:p-10 rounded-[1.5rem] sm:rounded-[3rem] border border-gray-100 shadow-sm">
        ...
        <button
          ...
          className="w-full sm:w-auto bg-primary text-white px-8 py-3.5 sm:py-4 rounded-xl sm:rounded-2xl font-black shadow-lg shadow-primary/20 hover:bg-gray-900 transition-all flex items-center justify-center gap-2 active:scale-95 outline-none focus:ring-4 focus:ring-primary/10 touch-manipulation text-[10px] sm:text-xs uppercase tracking-widest"
        >
          <Plus size={18} />
          <span>New Unit</span>
        </button>
            ...
            {filteredUoms.map((uom) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 mb-6 text-gray-400">
                  ...
                  <span className="font-mono text-[10px] sm:text-xs font-bold bg-gray-50/30 px-2 py-0.5 rounded text-gray-900">
                    {uom.code}
                  </span>
                </div>

                <div
                  className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-[9px] sm:text-[10px] font-black uppercase tracking-widest ${uom.isActive ? "bg-red-50 text-red-600" : "bg-red-50 text-red-500"}`}
                >
                  <span
                    className={`w-1.5 h-1.5 rounded-full ${uom.isActive ? "bg-red-500 animate-pulse" : "bg-red-500"}`}
                  />
            ...
            >
              ...
              <form onSubmit={handleSave} className="p-8 space-y-6">
                ...
                <button
                  type="submit"
                  disabled={saving}
                  className="w-full bg-primary text-white py-4 rounded-xl font-bold hover:bg-gray-900 transition-colors shadow-lg active:scale-95 disabled:opacity-70 flex justify-center items-center gap-2 focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none touch-manipulation"
                >
                  {saving ? (
